...
if(isset($_POST['submit']))
  {
	  ...
	  $pet_dis = $_POST['medhis'];
	  $pet_opdid = $_POST['dnames'];
    $insittedby = $_POST["sadmun"];


	  //$trn_date = date("Y-m-d H:i:s");
	  $pet_age = date_diff(date_create($pet_bd),date_create(date("Y-m-d")) );
	  $pet_age=$pet_age->format("%y");
	  ...
	  if($pet_type == "syst" | $pet_gender == "syg" | $pet_opdid == "setd" ){
		  $gestwterr = '<br/ ><div style="font-size:12px" align="center" class="alert alert-danger">Registration Unsuccessful, please fill all    	fields</div>';}else{
			  ...
			  if(isset($rowem['pet_con'])==$pet_cont){
    ...
    $query = "INSERT INTO `patient`
	  ( `pet_fn`, `pet_sn`, `pet_addr`, `pet_con`, `pet_em`, `pet_gen`, `pet_bd`, `pet_age`, `pet_bg`,`Pet_opdid`,`Insert_admunname`,`med_his`)
	  VALUES
	  ('$pet_fname','$pet_sname','$pet_add','$pet_cont','$pet_em','$pet_gender','$pet_bd',$pet_age,'$pet_type',$pet_opdid,'$insittedby','$pet_dis')";
