...
<div class="container">
<div class="row">
<div class="col-md-4 col-md-push-4 col-xs-12 alert alert-success text-center">Patients Registered on this forum are directly connected to the out-patient department (OPD) of the Hospital and Doctors.</div>
</div>
</div>
...
<?php
$count = 1;
$sel_query="SELECT `staffID`,`smfname`, `smlname`,`smspec` FROM `staff` WHERE `smtype` = 'Doctor'"; 
//selcet deoctor member id, deoctor first name and deoctor lastanme from staff table  
$result = mysqli_query($con,$sel_query);
while($row = mysqli_fetch_assoc($result)) { ?>
  <option data-tokens="" value="<?php echo $row["staffID"]; ?>"><?php echo $row["smfname"]; ?> <?php echo $row["smlname"]; ?> <?php echo '('; ?> <?php echo $row["smspec"]; ?> <?php echo ')'; ?></option>
<?php $count++; } ?>
</select>
  ...
  <tr>
    <td colspan="2" style="padding:5px"><div class="input-group col-md-12 col-xs-12 "> <span style="border-radius:0px; width:10px" class="input-group-addon" id="basic-addon1">+91</span><input  pattern="\d{10,10}"  maxlength="10" title="Invalid Mobile Number Use only Numbers and chacter count must be 10" name="tel" required  type="text" class="form-control" placeholder="Contact Number" style="border-radius:0px"></div><script type="text/javascript">
...
('<?php echo $emlduperr; ?>');</script></td>
  ...
  <tr>
    <td colspan="2" style="padding:5px;"><div class="input-group col-md-12 col-xs-12 "><select name="gender" required="required" title ="Select Your Gender"  class="form-control" placeholder="Select your Gender">
  ...
  <tr>
    <td colspan="2" style="padding:5px;"><div class="input-group col-md-12 col-xs-12 "><select name="bg" required="required" title="Select Your Blood Group"  class="form-control" placeholder="Select Blood group">
    ...
    <tr>
    <td colspan="2" style="padding:5px">
    <div class="input-group col-md-12 col-xs-12"><input pattern="[A-Za-z]{1,30}" title="Use Only Letters" required name="medhis" id="medhis"   type="text" class="form-control" placeholder="Any major diseases in past/Family History">
	</div>
    </td>
  </tr>
